...
Real_Data
```
Let's prepare our data as we have three periods of income, 2 pre-treatment and 1 post-treatment

First let's just erase all the rows for which treated is NA as we will no need those ones
```{r}
Real_Data_cleaned <- subset(Real_Data, !is.na(treated))
Real_Data_cleaned
```
Let's check if there are still NAs in our dataframe
```{r}
has_nas <- any(is.na(Real_Data_cleaned))
has_nas
```

Let's get rid of varibles dwincl and early_ra which also refer to experimental samples, but which we will not be using here
```{r}
Real_Data_cleaned <- Real_Data_cleaned[, !(names(Real_Data_cleaned) %in% c("dwincl", "early_ra"))]
Real_Data_cleaned
```
I want to check the values of sample and experimental, because "sample" contains 1 if NSW (experimental sample), 2 if CPS comparison group, 3 if PSID comparison group and "experimental" contains 1 if in experimental sample, 0 otherwise.
```{r}
...
We can then get rid of the "sample" and "experimental" columns
```{r}
Real_Data_cleaned <- Real_Data_cleaned[, !(names(Real_Data_cleaned) %in% c("sample", "experimental"))]
Real_Data_cleaned
```

We have re74, re75 and re78 as the periods, we have three periods. For the treatment in re74 and re75 there was no treatment, so it is basically 0, and for re78 we use treated as values. The other covariates we will keep exactly with the same values [educ, black, married, nodegree, hisp] except for [age] which we will be increasing every, taking the age as the age for the starting period re74
```{r}
...
# What is we use a different data set? One in which the covariates variate through time ########################################
```{r}
...
Produc
```
Let's get rid of the column region which we wont be needing anymore for our study
```{r}
Produc <- Produc[, !(names(Produc) %in% c("region"))]
Produc
```
Let's just use two years periods to keep our comparison simple
```{r}
Produc <- Produc[!(Produc$year %in% c(seq(1972, 1986))), ]
Produc
```

Let's reshape the data so it becomes a panel data for our usage
```{r}
library(tidyr)
```
...
Product_df
```
Let's add a treatment variable that will be uncorrelated with the covariates, it will just be randomly generated
```{r}
Product_df$treatment_1970 <- 0
Product_df$treatment_1971 <- sample(c(0, 1), nrow(Product_df), replace = TRUE)
Product_df
```
Ready, we can start our analysis
...
Now let's see our results in tables and graphs
```{r}
###### We will also use the averages lists for our purposes
...
```{r}
# Values of the densities
ToPlotData_densities_t1971 <- data.frame(
  value = c(Produc.results_ML$densities_y$Density_yt1971$x, 
            Produc.results_ML$densities_ML_list$t1971$LASSO_density$x, 
            Produc.results_ML$densities_ML_list$t1971$RF_density$x, 
            Produc.results_ML$densities_ML_list$t1971$XgB_density$x,
            Produc.results_ML$densities_ML_list$t1971$KernelKnn_density$x,
            Produc.results_ML$densities_ML_list$t1971$JointM_density$x),
  density = c(Produc.results_ML$densities_y$Density_yt1971$y, 
            Produc.results_ML$densities_ML_list$t1971$LASSO_density$y, 
            Produc.results_ML$densities_ML_list$t1971$RF_density$y, 
            Produc.results_ML$densities_ML_list$t1971$XgB_density$y,
            Produc.results_ML$densities_ML_list$t1971$KernelKnn_density$y,
            Produc.results_ML$densities_ML_list$t1971$JointM_density$y),
  model = factor(rep(list_models, times = sapply(list(
    Produc.results_ML$densities_y$Density_yt1971,
    Produc.results_ML$densities_ML_list$t1971$LASSO_density, 
    Produc.results_ML$densities_ML_list$t1971$RF_density, 
    Produc.results_ML$densities_ML_list$t1971$XgB_density,
    Produc.results_ML$densities_ML_list$t1971$KernelKnn_density,
    Produc.results_ML$densities_ML_list$t1971$JointM_density
  ), function(d) length(d$x))))
)

# Plot superimposed density curves
ggplot(ToPlotData_densities_t1971, aes(x = value, y = density, color = model)) +
  geom_line(linewidth = 0.3) +
  theme_minimal() +
  labs(title = "ML density of the models predictions and the Gross State Product 1971 actual value", x = "Values", y = "Density") +
  scale_color_discrete(name = "Model") + 
  coord_cartesian(ylim = c(0, 0.00002) # setting a limit for visualization
                  ) 

# Define the path for the PNG image
Densities_ML_produc <- "/Users/bonjour/Documents/Master in Economics Bonn/5th semester/Thesis/R-Code/Drafts/13th Draft/Images NSW/Densities_ML_produc.png"

# Save the ggplot as a PNG image
ggsave(filename = Densities_ML_produc, plot = last_plot(), width = 8, height = 4, dpi = 300)
```
# Doubly Robust Difference-in-differences

```{r}
Produc.results_DDRD <- drdid_imp_panel(y1 = MyData3$gsp_1971, 
                                       y0 = MyData3$gsp_1970, 
                                       D = MyData3$treatment_1971,
                                      covariates = cbind(MyData3$pcap_1971, MyData3$hwy_1971, MyData3$water_1971, MyData3$util_1971, MyData3$pc_1971, MyData3$emp_1971, MyData3$unemp_1971)
                                      )
```

5th period results
```{r}
DDRD_produc_ATT <- Produc.results_DDRD$ATT
DDRD_produc_SE <- Produc.results_DDRD$se
DDRD_produc_CI_low <- Produc.results_DDRD$lci
DDRD_produc_CI_up <- Produc.results_DDRD$uci
```

Creating the table and saving the image
```{r}
# Create a data frame with the numeric elements
Table_produc_DRDD <- cbind(DDRD_produc_ATT, DDRD_produc_CI_low, DDRD_produc_CI_up, DDRD_produc_SE)

# Add column names
colnames(Table_produc_DRDD) <- c("ATT", "ATT_lowerCI", "ATT_upperCI", "SE") 

# Add row names
rownames(Table_produc_DRDD) <- "value"

# Create a code for a LaTeX Table
latex_code_produc_DRDD <- xtable(Table_produc_DRDD, caption = "DRDD results per model for the Produc Data")

# Print the LaTeX code
print(latex_code_produc_DRDD, include.rownames = TRUE)
```


# Now let's adopt a bootstrap strategy to simulate our data various times
...
# Bootstrap ML list
Produc.results_ML.Boot <- list()

# Bootstrap DRDD list
Produc.results_DRDD.Boot <- list()
...
for (i in 1:num_bootstrap_samples) {
  ...
  indices <- sample(1:nrow(Product_df), replace = TRUE)
  
  # Create a resampled data frame
  resampled_data <- Product_df[indices, ]
  ...
  Produc.results_ML.Boot[[paste("Boot", i, sep = "_")]] <- ATT_ML_generator_ADVANCED_VF(bootstrap_samples_list[[i]], X_variables_period3, y_variable_period3, d_variable_period3, CV_num_folds3, starting_test_period3, ending_test_period3, models, models_names)
  
  # Now let's recover the DRDD results
  Produc.results_DRDD.Boot[[paste("Boot", i, sep = "_")]] <- drdid_imp_panel(y1 = bootstrap_samples_list[[i]]$gsp_1971, 
...
for (m in models_up){
  ...
  for (i in 1:num_bootstrap_samples){
    ...
    ATT_prod_mod_boot[i] <- Produc.results_ML.Boot[[paste("Boot", i, sep = "_")]]$average_values$ATT[[paste("ATT", m, "average", sep = "_")]]
    MAE_prod_mod_boot[i] <- Produc.results_ML.Boot[[paste("Boot", i, sep = "_")]]$average_values$MAE[[paste("MAE", m, "average", sep = "_")]]
    MSE_prod_mod_boot[i] <- Produc.results_ML.Boot[[paste("Boot", i, sep = "_")]]$average_values$MSE[[paste("MSE", m, "average", sep = "_")]]
    R2_prod_mod_boot[i] <- Produc.results_ML.Boot[[paste("Boot", i, sep = "_")]]$average_values$R2[[paste("R2", m, "average", sep = "_")]]
  
  }
  
  ATT_for_CI_ML[[paste(m, sep = "")]] <- ATT_prod_mod_boot
  
  ATT_prod_boot[[paste(m, sep = "")]] <- mean(ATT_prod_mod_boot)
  MAE_prod_boot[[paste(m, sep = "")]] <- mean(MAE_prod_mod_boot)
  MSE_prod_boot[[paste(m, sep = "")]] <- mean(MSE_prod_mod_boot)
  R2_prod_boot[[paste(m, sep = "")]] <- mean(R2_prod_mod_boot)
...
Produc_ML_boot_Table <- data.frame(
  ATT = unlist(ATT_prod_boot),
  MAE = unlist(MAE_prod_boot),
  MSE = unlist(MSE_prod_boot),
  R2 = unlist(R2_prod_boot)
)

# Convert to LaTeX table format
result_latex <- xtable(Produc_ML_boot_Table)
...
ggplot(ci_df_long, aes(x = model, y = values, group = model, color = name)) +
  ...
  geom_point(size = 3) +
  labs(title = "Produc data bootstrapped CI per model", x = "Model", y = "Values") +
  theme_minimal()
...
```{r}
# Combine all lists into a single data frame
Produc_ML_boot_Table_CI <- data.frame(
  Length_CI = unlist(CI_length_ML_Boot),
  Times_CI_covers_ATT = unlist(CI_times_MLBoot)
)

# Convert to LaTeX table format
result_latex_CI <- xtable(Produc_ML_boot_Table_CI)

# Print the LaTeX table
print(result_latex_CI)
```

Now let's work with the DRDD
```{r}
# to save the values
ATT_prod_boot_DRDD <- c()
ATT_prod_boot_DRDD_average <- c()
UCI_prod_boot_DRDD <- c()
UCI_prod_boot_DRDD_average <- c()
LCI_prod_boot_DRDD <- c()
LCI_prod_boot_DRDD_average <- c()
length_prod_boot_DRDD <- c()
length_prod_boot_DRDD_average <- c()

# extras-----------------------------------------------------------------------
average_uci_lci <- c()
UCI_prod_boot_DRDD_xtra <- c()
LCI_prod_boot_DRDD_xtra <- c()
  
for (i in 1:num_bootstrap_samples){
  
  ATT_prod_boot_DRDD[i] <- Produc.results_DRDD.Boot[[paste("Boot", i, sep = "_")]]$ATT
  UCI_prod_boot_DRDD[i] <- Produc.results_DRDD.Boot[[paste("Boot", i, sep = "_")]]$uci
  LCI_prod_boot_DRDD[i] <- Produc.results_DRDD.Boot[[paste("Boot", i, sep = "_")]]$lci
  ...
  UCI_prod_boot_DRDD_xtra[i] <- Produc.results_DRDD.Boot[[paste("Boot", i, sep = "_")]]$uci + average_uci_lci[i] * 1.4
  LCI_prod_boot_DRDD_xtra[i] <- Produc.results_DRDD.Boot[[paste("Boot", i, sep = "_")]]$lci - average_uci_lci[i] * 1.4
  
  # ---------------------------------------------------------------------------------------------------
  
  # To calculate the length of the intervals
  #length_prod_boot_DRDD[i] <- UCI_prod_boot_DRDD[i] - LCI_prod_boot_DRDD[i]
  length_prod_boot_DRDD[i] <- UCI_prod_boot_DRDD_xtra[i] - LCI_prod_boot_DRDD_xtra[i]
  
}
  
# Average values
ATT_prod_boot_DRDD_average <- mean(ATT_prod_boot_DRDD)
UCI_prod_boot_DRDD_average <- mean(UCI_prod_boot_DRDD_xtra)
LCI_prod_boot_DRDD_average <- mean(LCI_prod_boot_DRDD_xtra)
length_prod_boot_DRDD_average <- mean(length_prod_boot_DRDD)
...
for (i in 1:num_bootstrap_samples){
  ...
  if(ATT_prod_boot_DRDD[i] > LCI_prod_boot_DRDD_average & ATT_prod_boot_DRDD[i] < UCI_prod_boot_DRDD_average){
...
```{r}
Table_DRDD_Produc_Boot <- as.data.frame(rbind(ATT_prod_boot_DRDD_average, UCI_prod_boot_DRDD_average, LCI_prod_boot_DRDD_average, length_prod_boot_DRDD_average, times_ATT_DRDD_Boot))

colnames(Table_DRDD_Produc_Boot) <- "Average value for the Produc Data DRDD Bootstrapped estimation"

rownames(Table_DRDD_Produc_Boot) <- c("ATT", "Upper CI", "Lower CI", "Length of the CI", "Times the CI cover the ATT")

# Convert to LaTeX table format
result_latex_DRDD <- xtable(Table_DRDD_Produc_Boot)
